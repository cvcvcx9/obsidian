...
### 4. 향후 로드맵 (Hibernate 7.3 이후)

현재의 "공존 전략"은 **Hibernate 7.3**이 출시되어 Jackson 3를 공식적으로 네이티브 지원(Automatic Detection)하기 전까지만 유효한 한시적 베스트 프랙티스입니다. 7.3 버전으로 업데이트한 후에는 위 설정들을 모두 제거하고 순수 Jackson 3로 넘어갈 수 있습니다.

---

### 💡 다음 단계 제안

인프라 이슈가 완벽히 정리되었으니, 이제 원래 목표였던 **코드 생성 엔진 개발**에 집중할 시간입니다.

**이제 무엇을 도와드릴까요?**

- **Mustache 기반 코드 생성:** 사용자의 요청에 따라 `build.gradle` 소스를 생성하는 **`CodeGeneratorService`** 작성하기
    
- **DB 저장 로직:** 카프카 이벤트를 받아 `UsageLog` 엔티티로 DB에 저장하는 코드 완성하기
    

어떤 작업을 먼저 시작하고 싶으신가요? 바로 가이드를 드리겠습니다!